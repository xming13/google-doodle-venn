...
XMing.VennGoogle = new function() {
    ...
    this.initialize = function() {
        ...
        this.tween = function(prop, changeValue, endValue) {
            if ((changeValue < 0 && prop + changeValue > endValue) || (changeValue > 0 && prop + changeValue < endValue)) {
                prop += changeValue;
            } else {
                prop = endValue;
            }
        ...
        this.draw = function() {
            ...
            if (animEvents['startScreen'].isEnd &&
                ...
                if (bigLeftIconCenterX == BIG_LEFT_ICON_CENTER_X_INITIAL && bigRightIconCenterX == BIG_RIGHT_ICON_CENTER_X_INITIAL && tickRotate > 45) {
                    tickRotate = 0;
                ...
                for (var i = 0; i < smallLeftIcons.length; i++) {
                    ...
                    if (animEvents['leftIconExpand'].isStart && !animEvents['leftIconExpand'].isEnd) {
                        if (i == 0) {
                            icon.isStart = true;
                        }
                        if (icon.isStart) {
                    ...
                    if (animEvents['bigLeftIconSelected'].isStart && !animEvents['bigLeftIconSelected'].isEnd) {
                        if (i == 0) {
                            icon.isStart = true;
                        }
                        if (icon.isStart) {
                ...
                for (var i = 0; i < smallRightIcons.length; i++) {
                    ...
                    if (animEvents['rightIconExpand'].isStart && !animEvents['rightIconExpand'].isEnd) {
                        if (i == 0) {
                            icon.isStart = true;
                        }
                        if (icon.isStart) {
                    ...
                    if (animEvents['bigRightIconSelected'].isStart && !animEvents['bigRightIconSelected'].isEnd) {
                        if (i == 0) {
                            icon.isStart = true;
                        }
                        if (icon.isStart) {
        ...
        this.renderOverlappedCenter = function(leftCenterX, rightCenterX) {
            ...
            context.arc(leftCenterX, BIG_LEFT_ICON_CENTER_Y, BIG_ICON_RADIUS, -1 * radian, radian, false);
        ...
        this.onMouseMove = function(event) {
            ...
            var isHover = false;
            if (!animEvents['startScreen'].isEnd && mousePos.x >= BIG_LEFT_ICON_CENTER_X_FINAL && mousePos.x <= BIG_RIGHT_ICON_CENTER_X_FINAL && mousePos.y >= CENTER_Y - BIG_ICON_RADIUS && mousePos.y <= CENTER_Y + BIG_ICON_RADIUS) {
                isHover = true;
            }
            ...
            for (var i = 0; i < icons.length; i++) {
                var icon = icons[i];
                if ((icon.isTypeLeft() && !selectedLeftIcon) || (icon.isTypeRight() && selectedLeftIcon && !selectedRightIcon && animEvents['rightIconExpand'].isEnd) || icon.isTypeLink() || icon.isTypeReset() || icon.isTypeShare()) {
                    if (Math.sqrt(Math.pow(mousePos.x - icon.centerX, 2) + Math.pow(mousePos.y - icon.centerY, 2)) < icon.width / 2.0) {
                        icon.setHover(true);
                        isHover = true;
                    } else {
                        icon.setHover(false);
        ...
        this.onClick = function(event) {
            ...
            if (!animEvents['startScreen'].isEnd && mousePos.x >= BIG_LEFT_ICON_CENTER_X_FINAL && mousePos.y <= BIG_RIGHT_ICON_CENTER_X_FINAL) {
                animEvents['startScreen'].isEnd = true;
            }
            var icons = smallLeftIcons
                .concat(smallRightIcons)
            ...
            for (var i = 0; i < icons.length; i++) {
                ...
                if ((icon.isTypeLeft() && !selectedLeftIcon) || (icon.isTypeRight() && selectedLeftIcon && !selectedRightIcon) || icon.isTypeLink() || icon.isTypeReset() || icon.isTypeShare()) {
                    if (Math.sqrt(Math.pow(mousePos.x - icon.centerX, 2) + Math.pow(mousePos.y - icon.centerY, 2)) < icon.width / 2.0) {
                        if (icon.isTypeLeft()) {
